...
async def test_get_user(hass: HomeAssistant, aioclient_mock) -> None:
    ...
    aioclient_mock.get(
        ...
        json={"user": {"id": "user_abc123", "email": "test@example.com", "username": "testuser", "name": "Test User"}},
    )
...
async def test_get_teams(hass: HomeAssistant, aioclient_mock) -> None:
    ...
    aioclient_mock.get(
        ...
        json={"teams": [{"id": "team_abc", "slug": "my-team", "name": "My Team"}], "pagination": {"count": 1, "next": None, "prev": None}},
    )
    session = async_get_clientsession(hass)
    client = VercelApiClient(token="test_token", session=session)
...
async def test_get_projects(hass: HomeAssistant, aioclient_mock) -> None:
    ...
    aioclient_mock.get(
        ...
        json={"projects": [{"id": "prj_123", "name": "my-app", "framework": "nextjs", "nodeVersion": "20.x", "updatedAt": 1700000000000}], "pagination": {"count": 1, "next": None}},
    )
    session = async_get_clientsession(hass)
    client = VercelApiClient(token="test_token", session=session)
...
async def test_get_projects_with_team(hass: HomeAssistant, aioclient_mock) -> None:
    ...
    aioclient_mock.get(
        ...
        json={"projects": [{"id": "prj_456", "name": "team-app", "framework": "remix"}], "pagination": {"count": 1, "next": None}},
    )
    session = async_get_clientsession(hass)
...
async def test_get_deployments(hass: HomeAssistant, aioclient_mock) -> None:
    ...
    aioclient_mock.get(
        ...
        json={"deployments": [{"uid": "dpl_abc", "name": "my-app", "state": "READY", "target": "production", "created": 1700000000000, "ready": 1700000060000, "source": "git", "url": "my-app-abc.vercel.app", "inspectorUrl": "https://vercel.com/team/my-app/dpl_abc", "creator": {"username": "testuser"}, "meta": {"githubCommitMessage": "fix: bug"}, "isRollbackCandidate": True}], "pagination": {"count": 1, "next": None}},
    )
    session = async_get_clientsession(hass)
    client = VercelApiClient(token="test_token", session=session)
...
async def test_get_domains(hass: HomeAssistant, aioclient_mock) -> None:
    ...
    aioclient_mock.get(
        ...
        json={"domains": [{"name": "example.com", "verified": True, "expiresAt": 1800000000000, "renew": True}], "pagination": {"count": 1, "next": None}},
    )
    session = async_get_clientsession(hass)
    client = VercelApiClient(token="test_token", session=session)
...
async def test_get_project_env_vars(hass: HomeAssistant, aioclient_mock) -> None:
    ...
    aioclient_mock.get(
        ...
        json={"envs": [{"key": "DATABASE_URL", "type": "encrypted", "target": ["production"]}, {"key": "API_KEY", "type": "plain", "target": ["production", "preview"]}]},
    )
    session = async_get_clientsession(hass)
    client = VercelApiClient(token="test_token", session=session)
